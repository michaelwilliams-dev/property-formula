// server.js
// ISO Timestamp: 🕒 2025-07-30T16:30:00Z
...
// 🧠 Cosine similarity and top chunks
...
app.post('/api/blog-draft', async (req, res) => {
  ...
  try {
    ...
    console.log("🔎 OpenAI embedding response received.");
...
app.post('/api/blog-draft', async (req, res) => {
  ...
  try {
    ...
    // ✉️ Send email via Mailjet if requested
    if (email && email.includes('@')) {
      try {
        const pdfDoc = new PDFDocument();
        let pdfBuffer = Buffer.alloc(0);
